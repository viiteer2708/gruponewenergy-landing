/**
 * MEGA ENERGIA - Backend para formulario de tramitacion
 *
 * SETUP:
 * 1. Ve a https://script.google.com y crea un nuevo proyecto
 * 2. Pega este codigo en Code.gs
 * 3. Crea una carpeta en Google Drive llamada "Contratos Mega Energia"
 * 4. Copia el ID de esa carpeta (la parte de la URL despues de /folders/)
...
function doPost(e) {
  ...
  try {
    ...
    try {
      ...
      const folderName = refId + ' - ' + (data.titular || 'Sin titular') + ' - ' + timestamp;
...
function buildEmailHtml(data, fileLinks, folderUrl, refId) {
  const fields = [
    ...
    ['Titular / Razon Social', data.titular],
  ...
  if (fileLinks.length > 0) {
    filesHtml = '<h3 style="color:#0D9488;margin:24px 0 12px;font-size:15px">Documentacion adjunta</h3><ul style="list-style:none;padding:0">';
    fileLinks.forEach(function(f) {
      filesHtml += '<li style="margin:8px 0;padding:10px 14px;background:#f0fdfa;border:1px solid #e2e8f0;border-radius:8px;font-size:13px">' +
        '<a href="' + f.url + '" style="color:#14B8A6;font-weight:600;text-decoration:none">' + f.name + '</a>' +
        '<span style="color:#64748B;margin-left:8px">' + f.size + '</span>' +
      '</li>';
    });
    filesHtml += '</ul>';
    filesHtml += '<p style="margin-top:12px"><a href="' + folderUrl + '" style="color:#14B8A6;font-weight:600">Abrir carpeta en Google Drive</a></p>';
  }
  ...
  return '<div style="font-family:Arial,sans-serif;max-width:650px;margin:0 auto">' +
    '<div style="background:#0D9488;color:#fff;padding:20px 24px;border-radius:10px 10px 0 0">' +
      '<h2 style="margin:0;font-size:18px">Nuevo Contrato Mega Energia</h2>' +
