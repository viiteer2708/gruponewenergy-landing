...
function doPost(e) {
  ...
  // Honeypot relleno = bot (o, raro, autofill): éxito falso para no dar pistas,
  // pero CON rastro en el Sheet por si fuera un falso positivo
  if (data.hp) {
  ...
  // funcione también cuando un reintento llega con la primera ejecución en curso
    ...
    try {
      ...
      try {
        ...
        archivos.forEach(function(archivo) {
          ...
          // quedar accesible a "cualquiera con el enlace" (RGPD).
          fileLinks.push({
            name: safeName,
...
function logToSheet(refId, data, numArchivos, driveOk, emailSent, folderUrl, errorMsg) {
  try {
    ...
    if (!ss) {
      ss = SpreadsheetApp.create('Registro Tramitaciones MEGA');
    ...
    ss.getSheets()[0].appendRow([
      ...
      sheetSafe(data.cups),
      sheetSafe(data.tarifa),
      sheetSafe(data.tipo_suministro),
