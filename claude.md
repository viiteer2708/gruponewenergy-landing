...
- **Correos (17/08/2026)**: misma mecánica que GNEW — Brevo con `BREVO_SENDER = administracion@megaenergia.es` (dominio `megaenergia.es` autenticado; remitente dado de alta y ACTIVO el 17/08/2026) y clave `BREVO_API_KEY` en Propiedades del script o en `config-formulario.json` de la carpeta "Contratos Mega Energia" (creado el 17/08/2026, privado); respaldo GmailApp (`GMAIL_ALIAS = tramitaciones@megaenergia.es`, opcional). Sin enlaces de Drive, acuse de recibo al comercial con `Reply-To` = administracion@
- **Apps Script deployment**: acceso DEBE ser "Cualquier persona" (el front lee la respuesta JSON para confirmar el envío)
