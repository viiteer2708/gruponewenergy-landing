...
- **Correos (17/08/2026)** — capa `sendMail()` con dos vías: **A) Brevo API** (`sendViaBrevo`, clave en Propiedades del script `BREVO_API_KEY`, remitente `BREVO_SENDER = escaneos@gruponew.energy` que debe existir en Brevo con el dominio `gruponew.energy` autenticado: TXT `brevo-code`, `mail._domainkey`, `_dmarc`); **B) respaldo GmailApp** desde la cuenta que ejecuta (con `GMAIL_ALIAS = tramitaciones@gruponewenergy.es` si está como "Enviar como"; si no, la cuenta por defecto). Sin clave o si Brevo falla → Gmail, y queda anotado en la columna "Errores / notas" del Sheet. Aviso a escaneos@ con los documentos ADJUNTOS y **sin enlaces de Drive** (eran privados y solo generaban "solicitudes de acceso"), `Reply-To` = comercial. **Acuse de recibo al comercial** (`buildAcuseHtml`, sin IBAN/DNI) con `Reply-To` = escaneos@ y el aviso de dónde enviar la factura. Presupuesto de adjuntos `ATTACH_BUDGET_RAW` = 12MB reales (Brevo admite 20MB con base64; Gmail 25MB MIME); solo si algún archivo no cabe o falla el correo con adjuntos, la carpeta se comparte con escaneos@ (`shareFolderWithReceiver`) y el correo lleva el enlace. ⚠️ `UrlFetchApp` añade el scope `script.external_request`: la primera "Nueva versión" pide re-autorizar.
...
- **Correos (17/08/2026)**: misma mecánica que GNEW — Brevo con `BREVO_SENDER = administracion@megaenergia.es` (dominio `megaenergia.es` ya autenticado en Brevo; el remitente hay que darlo de alta) y clave `BREVO_API_KEY` en Propiedades del script; respaldo GmailApp (`GMAIL_ALIAS = tramitaciones@megaenergia.es`, opcional). Sin enlaces de Drive, acuse de recibo al comercial con `Reply-To` = administracion@
- **Apps Script deployment**: acceso DEBE ser "Cualquier persona" (el front lee la respuesta JSON para confirmar el envío)
